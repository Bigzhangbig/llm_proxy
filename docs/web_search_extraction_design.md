...
*   **解决方案**：
    引入一个**轻量过滤层 (Pre-Filter)**，单独配置一个高并发、超便宜、高吞吐的快速大模型（例如 `mimo-v2-flash` 或 `deepseek-v4-flash`）。
...
sequenceDiagram
    ...
    participant CheapLLM as 轻量模型 (mimo-v2-flash/deepseek-v4-flash)
...
export class WebSearchPipeline {
  ...
  constructor(config: PipelineConfig) {
    this.config = {
      maxSearchResults: 5,
      maxConcurrency: 3,
      cheapLlmModel: "mimo-v2-flash", // 默认选用高性价比 Flash 模型
      ...config
    };
  }
...
const webPipeline = new WebSearchPipeline({
  ...
  cheapLlmModel: "mimo-v2-flash", // 极速且极廉价的小米大模型 Flash 版本
...
1.  **极高性价比与低延迟**：通过本地 MinerU 进行预提取和廉价的 `mimo-v2-flash` 进行前置提炼，将原始动辄数万 Token 的网页内容，压缩为仅 1K-2K Token 的高价值干货，直接为主模型节省 90% 以上的输入上下文和推理 Token 成本，响应时间大幅缩短。
