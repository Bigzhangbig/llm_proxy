...
describe('keys', () => {
  describe('loadGeminiKeys', () => {
    it('loads keys from env vars', async () => {
      ...
      const { loadGeminiKeys } = await import('../search/keys')
    ...
    it('returns empty array when no keys set', async () => {
      const saved: Record<string, string | undefined> = {}
      for (let i = 1; i <= 10; i++) {
        const k = `GOOGLE_API_KEY_${i}`
        saved[k] = process.env[k]
  ...
  describe('loadMmxConfig', () => {
    it('returns config from env vars when config file absent', async () => {
      ...
      const { loadMmxConfig } = await import('../search/keys')
    ...
    it('defaults to cn region', async () => {
      const savedKey = process.env.MMX_API_KEY
      const savedRegion = process.env.MMX_REGION
      const savedHome = process.env.HOME
      process.env.HOME = '/nonexistent'
